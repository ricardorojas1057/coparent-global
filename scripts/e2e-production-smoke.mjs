...
const password = process.env.COPARENT_QA_PASSWORD;
const parent1Email = process.env.COPARENT_QA_PARENT1_EMAIL;
const parent2Email = process.env.COPARENT_QA_PARENT2_EMAIL;

if (!password || !parent1Email || !parent2Email) {
  throw new Error(
    'Configure COPARENT_QA_PARENT1_EMAIL, COPARENT_QA_PARENT2_EMAIL y COPARENT_QA_PASSWORD con cuentas QA verificadas.',
  );
}
...
const parent1 = await request('/auth/login', {
  method: 'POST',
  body: { email: parent1Email, password },
});
const parent2 = await request('/auth/login', {
  method: 'POST',
  body: { email: parent2Email, password },
});
check(parent1.accessToken && parent2.accessToken, 'Both verified QA accounts must receive access tokens.');
...
const [parent1Families, parent2Families, messages, events, report, summary, subscription] = await Promise.all([
  ...
  request(`/families/${family.id}/subscription`, { token: parent1.accessToken }),
]);
...
check(subscription.effectivePlan === 'PREMIUM', 'New family must start with a Premium trial.');
check(subscription.billing?.familyWide === true, 'Subscription must apply to the whole family.');
const requestedSubscription = await request(`/families/${family.id}/subscription/request`, {
  token: parent1.accessToken,
  method: 'PATCH',
  body: { plan: 'PLUS' },
});
check(requestedSubscription.subscription.requestedPlan === 'PLUS', 'Plan request was not recorded.');
check(requestedSubscription.billing?.googlePlayReady === false, 'Billing must stay disabled until Play verification exists.');
  ...
  JSON.stringify(
    {
      ...
      checks: [
        ...
        'family-wide Premium trial and subscription catalog',
        'safe plan request without an unverified charge',
      ],
    },
    null,
    2,
